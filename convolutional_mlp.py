...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,
    ...
    layer0_input = x.reshape((batch_size, 2, 15, 15))

    layer0 = LeNetConvPoolLayer(
        rng,
        input=layer0_input,
        image_shape=(batch_size, 2, 15, 15),
    ...
    layer2 = LeNetConvPoolLayer(
        rng,
        input=layer1.output,
        image_shape=(batch_size, nkerns[1], 15, 15),
        filter_shape=(1, nkerns[1], 1, 1),
        poolsize=(1, 1)
        )
    
    layer2_output = T.nnet.softmax(layer2.output.flatten(2))
    #layer2_output = layer2.output.flatten(2)
    # cost is the negative log likelihood
    cost = -T.mean(T.log(layer2_output[T.arange(y.shape[0]), y]))
    error = T.mean(T.neq(T.argmax(layer2_output, axis=1), y))
